...
class Voo
{
    ...
    public function buscaVoo($dataIda, $dataVolta)
    {
        try {
            ...
            valorVoo , dataHoraChegada , imagemVoo , assentosDisponiveis FROM  viagens360 . tabelavoos where origemVOO= :origemVoo AND destinoVOO = :destinoVoo AND qtdPassagens > 0
            ...
            valorVoo , dataHoraChegada , imagemVoo , assentosDisponiveis FROM  viagens360 . tabelavoos where origemVOO= :destinoVoo AND destinoVOO = :origemVoo AND qtdPassagens > 0
    ...
    public function listaDadosVooCompra($idVooIda, $idVooVolta)
    {
        try {
            $conn = Conexao::conectar();
            $sql = $conn->prepare("SELECT  idVoo , classeVoo , origemVOO , destinoVOO , dataHoraPartida , numVoo , modeloAeronave , 
            valorVoo , dataHoraChegada , imagemVoo , assentosDisponiveis, codReserva FROM  viagens360 . tabelavoos where idVoo in (:idVooIda, :idVooVolta) ");
    ...
    public function finalizaCompra($idUsuario)
    {
        ...
        try {
            ...
            $sql = $conn->prepare("INSERT INTO viagens360.tabelareserva (codReservaVoo, idVoo, idUsuario, assentoReservado, statusReserva) 
            ...
            $sql2 = $conn->prepare("UPDATE viagens360.tabelavoos SET qtdPassagens = (qtdPassagens - 1) WHERE idVoo = :idVoo");
    ...
    public function pesquisaVoo()
    {
        try {
            $conn = Conexao::conectar();
            $sql = $conn->prepare("SELECT  origemVOO FROM  viagens360.tabelavoos group by origemVOO ");
            $sql->execute();
