...
class Checkin
{
    ...
    public function pesquisaCheckin()
    {
        ...
        if ($codReserva && $codUsuario) {
            try {
                ...
                FROM viagens360.tabelavoos tv 
                JOIN viagens360.tabelareserva tr ON tv.idVoo = tr.idVoo JOIN viagens360.tabelausuario tu ON tu.idUsuario = tr.idUsuario 
    ...
    public function validaCheckin()
    {
        ...
        try {
            $conn = Conexao::conectar();
            $sql = $conn->prepare("UPDATE viagens360.tabelareserva SET validaCheckin = 1 WHERE codReservaVoo LIKE :codReserva;");
